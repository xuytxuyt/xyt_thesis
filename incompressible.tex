...
图\ref{ch_4:fig:mf}为一阶多项式基函数和二阶多项式基函数的无网格形函数图。从图中可以看出，无网格形函数具有全域高阶连续光滑的特点。
\begin{figure}[!h]
    \centering
        \begin{tabular}{c@{\hspace{0pt}}c}
            $p=1$& $p=2$ \\
            \includegraphics[width=0.5\textwidth]{figures/ch_4/shapefunction11.png} &
            \includegraphics[width=0.5\textwidth]{figures/ch_4/shapefunction21.png} \\
            \includegraphics[width=0.5\textwidth]{figures/ch_4/shapefunction12.png} &
            \includegraphics[width=0.5\textwidth]{figures/ch_4/shapefunction22.png} \\
        \end{tabular}
        \caption{二维无网格形函数}\label{ch_4:fig:mf}
\end{figure}

无网格形函数近似的压力$p_h$的变分可以表示为：
\begin{equation}
    \delta p_h(\boldsymbol x) = \sum_{K=1}^{n_p} \Psi_K(\boldsymbol x) \delta p_K
\end{equation}

将采用有限元离散的位移$\boldsymbol{u}$和采用无网格离散的压力$p$回代到混合离散方程\eqref{ch_2:eq:weak_mix}，且根据$\delta\boldsymbol{u}_h$和$\delta p_h$的任意性，可得离散控制方程：
\begin{equation}
    \begin{bmatrix}
    ...
    \end{Bmatrix}
\end{equation}
其中各矩阵的表达式见式\eqref{ch_2:eq:equilibrium_up}、\eqref{ch_2:eq:equilibrium_pp}。
...
以Tri3单元为例，图\eqref{ch_3:eq:best}(a)中的红色曲线反映了在位移节点数$n_u$固定的情况下，压力节点数$n_p$的变化对LBB稳定系数$\beta$的影响，颜色由浅到深表示位移节点数逐渐增加。
横坐标为压力节点数的变化，纵坐标为LBB稳定系数$\beta$的值。
$\beta$值越往下代表$\beta$越接近于零，表明混合方案不稳定。$\beta$值越靠上表示该约束比例下的混合方案稳定性越好(满足LBB稳定性条件)。
图中四个竖向虚线分别表示四个不同位移节点数$n_u$对应的稳定压力节点数$n_s$。
测试结果表明：当压力节点数$n_p$小于$n_s$(位于虚线左侧)时，离散方案趋于稳定，满足LBB稳定性条件；
而当$n_p$大于$n_s$(位于虚线右侧)时，离散方案则表现出不稳定性，无法满足LBB稳定性条件。
值得注意的是，传统有限元混合离散方案中采用的最优约束比($r=2$)对应的$\beta$值均位于虚线右侧，这表明传统有限元混合离散方案不满足LBB稳定性条件。
如图\ref{ch_4:fig:infsup_convergence}(b)、(c)、(d)所示，Tri6单元、Quad4单元和Quad8单元的测试结果与Tri3单元的基本一致。为保证混合离散方案在满足LBB稳定性条件的同时，避免使用两套网格进行离散而增加计算量，本研究基于上述测试结果，选取在虚线左边的压力节点数作为离散方案，提出了如图\ref{ch_4:fig:mix_scheme}所示的免体积自锁有限元无网格混合离散方案。该方案的压力节点选取直接基于位移节点，从而实现了方案的简易性与稳定性。
...
\begin{figure}[!h]
    ...
    \begin{subcaptiongroup}
        \begin{tabular}{c@{\hspace{0pt}}c}
        \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_4/cook_tri3_32_2145.png}}
        & \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_4/cook_tri3_32_561.png}}\\
        Tri3($r=2$)&Tri3($r=r_{opt}$)  \\
        \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_4/cook_tri6_32_8385.png}}
        & \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_4/cook_tri6_32_2145.png}} \\
        Tri6($r=2$)&Tri6($r=r_{opt}$)  \\
        \end{tabular}
    \end{subcaptiongroup}
    \caption{Cook membrane 问题三角形单元压力云图}\label{ch_4:fig:Cook_membrane_p}
\end{figure}
...
\begin{figure}[!h]
    ...
    \begin{subcaptiongroup}
        \begin{tabular}{c@{\hspace{0pt}}c}
        \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_4/cook_quad_32_2145.png}}
        & \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_4/cook_quad_32_561.png}}\\
        Quad4($r=2$)&Quad4($r=r_{opt}$)  \\
        \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_4/cook_quad8_32_6337.png}}
        & \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_4/cook_quad8_32_2145.png}} \\
        Quad8($r=2$)&Quad8($r=r_{opt}$)  \\
        \end{tabular}
    \end{subcaptiongroup}
    \caption{Cook membrane 问题四边形单元压力云图}\label{ch_4:fig:Cook_membrane_p2}
\end{figure}
\section{小结}
