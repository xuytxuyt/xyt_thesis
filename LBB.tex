...
\begin{equation}\label{ch_3:eq:base1}
    ...
    \end{Bmatrix}
\end{equation}
令$n_s = \dim(V_{n_u}\setminus \ker \mathcal P)$，此时$n_s=1$。当$n_p\leq n_s=1$时，$n_s$为最小特征值。这种情况下$\beta_s>0$，即满足LBB稳定性条件。当$n_p>n_s=1$时$\beta_s=0$，不满足LBB稳定性条件。
...
\begin{equation}\label{ch_3:eq:base2}
    ...
    \end{Bmatrix}
\end{equation}
在这种情况下，$n_s=3$。当$n_p\leq n_s=3$时，$n_s$为最小特征值。这种情况下$\beta_s>0$，即满足LBB稳定性条件。当$n_p>n_s=3$时，$\beta_s=0$，不满足LBB稳定性条件。
