...
本章针对中厚板的剪切自锁问题，提出免剪切自锁的有限元无网格混合离散方案。首先，基于中厚板的本构特性，分析了传统有限元分析过程中产生剪切自锁现象的原因。基于LBB稳定系数误差估计和多项式空间，确定剪切自锁问题的最优约束比取值范围。随后，确定剪切自锁问题的有限元无网格混合离散方案，并通过典型板结构算例验证其正确性。
\section{中厚板问题有限元法}    
...
\begin{equation}\label{ch_5:eq:ritz_penalty_mindlin}
    \text{find}\; \boldsymbol u^h \in V_h, \quad
    a(\delta \boldsymbol u^h, \boldsymbol u^h) +
    a^s(\delta \boldsymbol u^h, \boldsymbol u^h) = 
    f(\delta \boldsymbol u^h)
    ,\quad \forall \delta \boldsymbol u^h \in V
\end{equation}
其中，$\boldsymbol u^h = \{ w^h, \varphi_1^h, \varphi_2^h \}^T$。
...
需要指出，在中厚板问题中当板的厚度减小时，$h\rightarrow 0$，$\boldsymbol K^s$中的材料系数将远大于$\boldsymbol K^b$，导致$\boldsymbol K^s$中的模态被约束。在此问题中$\boldsymbol K^s$即为罚函数项。
...
\begin{equation}\label{ch_5:eq:potential_energy_mixed}
    \begin{split} 
        \Pi(\boldsymbol u,\boldsymbol{Q})&=\int_{\bar\Omega}\frac{1}{2}\varepsilon_{ij}\sigma_{ij} d\Omega+\frac{1}{2}\int_{\Omega}Q_{\alpha}(\gamma_{\alpha}-\frac{Q_{\alpha}}{kGh})d\Omega-\int_{\Gamma^{t}} u_{i}t_{i}d\Gamma-\int_{\Omega} u_{i}b_{i}d\Omega\\
...
对式\eqref{ch_5:eq:potential_energy_mixed}进行变分可得利兹--伽辽金弱问题：
...
其中
\begin{align}
    a(\delta \boldsymbol u, \boldsymbol u) &= \int_{\Omega}\delta\kappa_{\alpha \beta}C_{\alpha\beta\gamma\eta}\kappa_{\alpha \beta}d\Omega \\
    ...
    f(\delta \boldsymbol u)&=\int_{\Gamma_{Q}}\delta{w}{\bar Q}d\Gamma-\int_{\Omega} \delta{w}\bar{q}d\Omega-\int_{\Gamma_{M}}\delta\varphi_{\alpha}{{\bar M}_{\alpha}}d\Gamma
\end{align}

采用伽辽金法进行求解时，挠度$w$、转角$\boldsymbol{\varphi}$与剪切应力$\boldsymbol{Q}$可以采用不同的离散方式进行近似，形成混合离散框架。挠度$w$和转角$\boldsymbol{\varphi}$采用有限元形函数\eqref{ch_5:eq:w_h}，\eqref{ch_5:eq:varphi_h}进行近似，剪切应力$\boldsymbol{Q}$通过再生核无网格形函数\eqref{ch_4:eq:mfshapefunction}进行近似，近似的剪切应力$\boldsymbol{Q}^h$及其变分可表示为：
\begin{equation}\label{ch_5:eq:Q_h}
    \boldsymbol Q^h(\boldsymbol x) = \sum_{K=1}^{n_q} \Psi_K(\boldsymbol x) \boldsymbol{q}_{K},\quad \delta
    \boldsymbol Q^h(\boldsymbol x) = \sum_{K=1}^{n_q} \Psi_K(\boldsymbol x) \delta \boldsymbol{q}_{ K}
\end{equation}
...
在二维弹性中厚板问题中，对于维数为$n_u=3$的线性多项式空间$P_3$，对应的挠度空间$V_3$由下式给出：
...
\begin{equation}\label{ch_5:eq:base2}
    ...
    \end{Bmatrix}
\end{equation}
此时，$n_s=6$。当$n_q=n_s=6$时，$\beta_s>0$，即满足LBB稳定性条件。当$n_q>n_s=6$时，$\beta_s=0$，不满足LBB稳定性条件。更多的多项式阶数下$n_u$与$n_s$的关系如表\ref{ch_5:tab:constraint}所示。
\begin{table}[!h]
    \centering
    ...
    \begin{tabular}{cccc}
        ...
        $n_u$ & $3n_u$ & $\left \lfloor\frac{\sqrt{1+8n_u}-3}{2}\right \rfloor$ & $\frac{(n+1)(n+2)}{2}$ \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{equation}\label{ch_5:eq:best}
    r_{opt} \ge \frac{3n_u}{n_s},\quad n_s = \frac{(n + 1)(n + 2)}{2},\quad n =\left \lfloor\frac{\sqrt{1+8n_u}-3}{2}\right \rfloor
\end{equation}

需要注意的是，上面提到的挠度自由度个数$3n_u$为施加本质边界条件之后的挠度自由度个数。
...
%     \begin{subcaptiongroup}
%         \begin{tabular}{c@{\hspace{0pt}}c@{\hspace{0pt}}c}
%          \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_5/SquarePlate_mix_tri6_q1_16_16.png}}
...
%         \begin{tabular}{c@{\hspace{0pt}}c@{\hspace{0pt}}c}
%          传统混合离散方案&所提混合离散方案& \\
%          \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_5/MorleysAcuteSkewPlate_tri6_q1_16_16.png}}
...
%          \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_5/MorleysAcuteSkewPlate_quad8_q1_16_16.png}}
