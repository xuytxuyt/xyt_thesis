\chapter{剪切自锁问题的有限元无网格混合离散分析方法}
本章针对中厚板剪切自锁问题混合离散框架，建立剪切自锁问题的LBB稳定系数估计表达式，并确定满足剪切自锁问题LBB稳定性条件下的最优约束比。同时，将有限元无网格混合离散分析方法推广至中厚板问题，通过LBB稳定性条件特征问题验证LBB稳定系数估计的有效性，确定最优约束比下剪切力节点的布置方案，并用中厚板算例验证所提方法计算精度和稳定性。

\section{剪切自锁问题}    
\subsection{罚函数法}
考虑如图\ref{mindlin_picture}所示中厚板，其中$\Omega$为板中面，板厚度为$h$。在Mindlin假设下\cite{mindlin1951}，中厚板考虑横向剪切变形，相应的混合控制方程由下式给出：
...
\begin{figure}[!h]
    \centering 
        ...
        \caption{中厚板问题及边界条件示意图}\label{mindlin_picture}
\end{figure}
...
\begin{equation}\label{ch_5:eq:mindlin_M}
    M_{\alpha \beta}=-\frac{h^3}{12}D_{\alpha \beta \gamma\eta}\kappa_{\gamma\eta}=\frac{h^3}{12}C_{\alpha \beta \gamma\eta}\varphi_{\gamma,\eta}
\end{equation}
\begin{equation}\label{ch_5:eq:mindlin_Q}
    Q_{\alpha}=k\frac{Eh}{2(1+\nu)}\gamma_\beta =hkG(-\varphi_\beta+w_{,\beta})
\end{equation}
...
\begin{equation}\label{ch_5:eq:potential_energy}
    \begin{split} 
        \Pi(\boldsymbol{u})&=\frac{1}{2}\int_{\Omega}\kappa_{\alpha\beta}M_{\alpha\beta}d\Omega+\frac{1}{2}\int_{\Omega}\gamma_{\alpha}Q_{\alpha}d\Omega\\
        &+\int_{\Gamma_{M}}\varphi_{\alpha}{\bar{M}_{\alpha}}d\Gamma-\int_{\Gamma_{Q}}{w}\bar {Q}d\Gamma-\int_{\Omega} w\bar{q}d\Omega
    \end{split}
\end{equation}
其中，$\boldsymbol u = \{w, \varphi_1, \varphi_2\}^T$。对式\eqref{ch_5:eq:potential_energy}进行变分可得伽辽金弱形式：
\begin{flalign}
...
\begin{equation}\label{ch_5:eq:weak_penalty_mindlin}
    ...
    \end{aligned}
    ,\quad \forall\boldsymbol u  \in V
\end{equation}

对于考虑横向剪切变形的中厚板，根据中厚板的本构关系\eqref{ch_5:eq:mindlin_M}，\eqref{ch_5:eq:mindlin_Q}，当其厚度减小$h\rightarrow 0$，$h \gg h^3$将导致弯矩$M_{\alpha \beta}$减小的速度远大于剪力$Q_{\alpha}$，使得$Q_{\alpha}\gg M_{\alpha \beta}$。从式\eqref{ch_5:eq:weak_penalty_mindlin}中可以看出当$Q_{\alpha}\gg M_{\alpha \beta}$，剪切应变$\boldsymbol{\gamma}$被约束，导致挠度和转角约束为$w_{,\alpha} = \varphi_\alpha$。
...
\begin{equation}
    r = \frac{3\times n_u}{2\times n_q}, \quad 
    \begin{cases}
        r > 3:2 & \text{过少的约束} \\
        r = 3:2 & \text{最优} \\
        r < 3:2 & \text{过多的约束} \\
...
对于中厚板剪切自锁问题，同样可以采用LBB稳定系数估计式\eqref{ch_3:eq:r3}计算最优约束比。此时，挠度自由度$\bar n_u$与应力自由度$n_q$的数量分别表示为：
\begin{equation}
 3\times \bar n_u = \dim V_{n_u},\, 2\times n_q = \dim(V_{\bar{n}_u}\setminus\ker \mathcal P_h \mathcal I_h)
\end{equation}
接下来以二维问题为例，讨论挠度自由度数量$\bar{n}_u$，剪切应力自由度数量$n_q$和$\beta_s$之间的关系。
...
\begin{equation}\label{ch_5:eq:base}
    ...
    \end{Bmatrix}
\end{equation}
在这种情况下，$n_s=6$。当$n_q\leq n_s/2=3$时，$\beta_s>0$，即满足LBB稳定性条件。当$n_q>n_s/2=3$时$\beta_s=0$，不满足LBB稳定性条件。
...
\begin{equation}\label{ch_5:eq:base2}
    ...
    \end{Bmatrix}
\end{equation}
此时，$n_s=12$。当$n_q\leq n_s/2=6$时，$\beta_s>0$，即满足LBB稳定性条件。当$n_q>n_s/2=6$时，$\beta_s=0$，不满足LBB稳定性条件。更多的多项式阶数下$\bar{n}_u$与$n_s$的关系如表\ref{ch_5:tab:constraint}所示。
\begin{table}[!h]
    \centering
...
\begin{equation}\label{ch_5:eq:best}
    n_p\leq \frac{(n+1)(n+2)}{2},\quad n = \lfloor\frac{\sqrt{1+8\bar{n}_u}-3}{2}\rfloor
\end{equation}
...
选取Tri3和Tri6两种离散单元，分别对挠度节点数为$n_u=$81、289、1089、4225进行了系统分析。图\ref{ch_5:fig:plate_l2}展示了相应的位移误差和应力误差的分析结果，其中横坐标为剪切应力节点数的变化，竖向虚线表示基于LBB稳定系数估计得到的稳定剪切应力节点数量$n_p=n_s$。从图中可以得出以下结论：对于位移误差分析，剪切应力节点的数量$n_q$在一个较宽的范围内均能保持较低的挠度误差水平；对于应力误差分析，当$n_q$小于最优值时，应力误差显著降低，结果验证了离散方案在$n_q$小于最优值时能获得高精度的数值解。值得注意的是，与体积不可压问题不同，中厚板剪切自锁问题的约束比增加到一定值后，误差会迅速增大。

基于上述讨论结果以及LBB稳定误差估计的最优约束比取值范围，同时为了降低剪切应力节点布置的计算成本，本文提出了一种如图\ref{ch_5:fig:platemsh}所示的混合离散方案。该方案中，剪切应力节点数量$n_q$由挠度节点数量$n_u$减去最外围可能受到边界条件约束的节点数量确定，因此其约束自由度关系满足$n_q\leq \bar{n}_u$。

图\ref{ch_5:fig:plate_tri_q}为固支方板问题中Tri3单元和Tri6单元的应力云图。从图中可明显观察到，传统混合离散方案存在显著的应力振荡现象，而本文提出的混合离散方案则有效的缓解了这一现象。
图\ref{ch_5:fig:SquarePlate_tri}为三角形单元位移和压力误差对比图。从图中可以看出，对于位移误差，$r=3:2$时Tri6单元不能达到理论误差收敛率，其他方案均能达到。对于应力误差，Tri3单元能达到理论误差收敛率，且本文所提方案的误差优于$r=3:2$时的误差，而Tri6单元均不能达到理论误差收敛率。
图\ref{ch_5:fig:plate_quad_q}为四边形单元的压力云图。结果表明，对于四边形单元，$r=3:2$时的应力云图存在应力振荡现象，而本文提出的离散方案缓解了应力振荡现象。上述结果充分验证了本文所提最优约束比范围的正确性，同时证明了混合离散方案在缓解应力振荡现象方面的有效性。
\begin{figure}[H]
    \centering
...
图\ref{ch_5:fig:plate_quad_q}为固支方板问题Quad4和Quad8单元的应力云图。从图中可以观察到，对于四边形单元$r=2$时存在应力振荡现象，而本文提出的混合离散方案则有效的缓解了这一现象。图\ref{ch_5:fig:SquarePlate_quad}为四边形单元位移和压力误差对比图。
从图中可以看出，对于挠度误差，所有方案均能达到理论误差收敛率，且两种方案具有相近的应力误差。对于应力误差，所有方案均能达到理论误差收敛率，但本文所提方案的应力误差均优于$r=2$时的应力误差。
\begin{figure}[!h]
    \centering
        \begin{tabular}{c@{\hspace{24pt}}c}
            \includegraphics[width=0.45\textwidth]{figures/ch_5/SquarePlate_tri3_8_msh.png} &
            \includegraphics[width=0.45\textwidth]{figures/ch_5/SquarePlate_quad4_8_msh.png} \\
            mix--Tri3& mix--Quad4\\
            \includegraphics[width=0.45\textwidth]{figures/ch_5/SquarePlate_tri6_8_msh.png} &
            \includegraphics[width=0.45\textwidth]{figures/ch_5/SquarePlate_quad8_8_msh.png} \\
            mix--Tri6& mix--Quad8\\
...
\begin{figure}[!h]
    ...
    \begin{subcaptiongroup}
        \begin{tabular}{c@{\hspace{0pt}}c}
         \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_5/SquarePlate_mix_tri3_q1_64_64.png}}
         & \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_5/SquarePlate_mix_tri3_q11_64_64.png}}\\
         mix-Tri3($r=3:2$)&mix-Tri3($r=r_{opt}$)\\
         \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_5/SquarePlate_mix_tri6_q1_32_32.png}}
        & \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_5/SquarePlate_mix_tri6_q11_32_32.png}} \\
...
\begin{figure}[!h]
    ...
    \begin{subcaptiongroup}
        \begin{tabular}{c@{\hspace{5pt}}c}
          \raisebox{-0.8\height}{\includegraphics[width=0.5\textwidth]{figures/ch_5/u_tri.png}}
        & \raisebox{-0.8\height}{\includegraphics[width=0.5\textwidth]{figures/ch_5/q_tri.png}}\\
        \end{tabular}
    \end{subcaptiongroup}
    \caption{固支方板问题三角形单元误差对比}\label{ch_5:fig:SquarePlate_tri}
\end{figure}
...
\begin{figure}[!h]
    ...
    \begin{subcaptiongroup}
        \begin{tabular}{c@{\hspace{0pt}}c}
            \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_5/SquarePlate_mix_quad4_q1_64_64.png}}
            & \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_5/SquarePlate_mix_quad4_q11_64_64.png}}\\
            mix-Quad4($r=3:2$)&mix-Quad4($r=r_{opt}$)\\
            \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_5/SquarePlate_mix_quad8_q1_32_32.png}}
           & \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_5/SquarePlate_mix_quad8_q11_32_32.png}} \\
...
\begin{figure}[!h]
    ...
    \begin{subcaptiongroup}
        \begin{tabular}{c@{\hspace{5pt}}c}
          \raisebox{-0.8\height}{\includegraphics[width=0.5\textwidth]{figures/ch_5/u_quad.png}}
        & \raisebox{-0.8\height}{\includegraphics[width=0.5\textwidth]{figures/ch_5/q_quad.png}}\\
        \end{tabular}
    \end{subcaptiongroup}
    \caption{固支方板问题四边形单元误差对比}\label{ch_5:fig:SquarePlate_quad}
\end{figure}
...
\begin{figure}[!h]
    \centering 
        \includegraphics[scale=0.8]{figures/ch_5/xie plate.png}
        \caption{Razzaque斜板问题模型}\label{ch_5:fig:Razzaque_plate}
\end{figure}

根据上文提出的混合离散方案，Razzaque斜板问题的混合离散方案如图\ref{ch_5:fig:Razzaque_plate_msh}所示。其约束自由度关系满足$n_q<\bar n_u$。
\begin{figure}[!h]
    \centering
        \begin{tabular}{c@{\hspace{24pt}}c}
            \includegraphics[width=0.35\textwidth]{figures/ch_5/SkewPlate_tri3_8_msh.png} &
            \includegraphics[width=0.35\textwidth]{figures/ch_5/SkewPlate_quad4_8_msh.png} \\
            mix--Tri3& mix--Quad4\\
            \includegraphics[width=0.35\textwidth]{figures/ch_5/SkewPlate_tri6_8_msh.png} &
            \includegraphics[width=0.35\textwidth]{figures/ch_5/SkewPlate_quad8_8_msh.png} \\
            mix--Tri6& mix--Quad8\\
            \raisebox{-0.3\height}{\includegraphics[width=14pt]{figures/legend_u.png}} ：挠度节点 &
            \raisebox{-0.3\height}{\includegraphics[width=14pt]{figures/legend_q.png}} ：剪切应力节点 
        \end{tabular}
...
图\ref{ch_5:fig:Razzaque_plate_q}和图\ref{ch_5:fig:Razzaque_plate_q2}为Razzaque斜板问题应力云图。从图中可以看出，无论是线性单元还是二次单元，$r=3:2$时都存在显著的应力振荡现象，而本文缩题混合离散方案则有效的缓解了应力振荡现象。
...
\begin{figure}[!h]
    ...
    \begin{subcaptiongroup}
        \begin{tabular}{c@{\hspace{0pt}}c}
           ...
           & \raisebox{-0.7\height}{\includegraphics[width=0.4\textwidth]{figures/ch_5/MorleysAcuteSkewPlate_tri6_q11_32_32.png}} \\
           mix-Tri6($r=3:2$)&mix-Tri6($r=r_{opt}$) \\
        \end{tabular}
    \end{subcaptiongroup}
...
\begin{figure}[!h]
    ...
    \begin{subcaptiongroup}
        \begin{tabular}{c@{\hspace{0pt}}c}
           ...
           & \raisebox{-0.7\height}{\includegraphics[width=0.4\textwidth]{figures/ch_5/MorleysAcuteSkewPlate_quad8_q11_32_32.png}} \\
           mix-Quad8($r=3:2$)&mix-Quad8($r=r_{opt}$) \\
        \end{tabular}
    \end{subcaptiongroup}
...
\begin{figure}[!h]
    \centering
        \begin{tabular}{c@{\hspace{24pt}}c}
            \includegraphics[width=0.27\textwidth]{figures/ch_5/Circular_tri3_8_msh.png} &
            \includegraphics[width=0.27\textwidth]{figures/ch_5/Circular_quad4_8_msh.png} \\\
            mix--Tri3& mix--Quad4\\
            \includegraphics[width=0.27\textwidth]{figures/ch_5/Circular_tri6_8_msh.png} &
            \includegraphics[width=0.27\textwidth]{figures/ch_5/Circular_quad8_8_msh.png} \\
            mix--Tri6& mix--Quad8\\
            \raisebox{-0.3\height}{\includegraphics[width=14pt]{figures/legend_u.png}} ：挠度节点 &
            \raisebox{-0.3\height}{\includegraphics[width=14pt]{figures/legend_q.png}} ：剪切应力节点 
        \end{tabular}
...
图\ref{ch_5:fig:Circular_q}和图\ref{ch_5:fig:Circular_q2}为简支圆板问题应力云图。从图中可以看出，无论是线性单元还是二次单元，$r=2$时都存在显著的应力振荡现象，而本文所提混合离散方案则有效的缓解了应力振荡现象。
...
\begin{figure}[!h]
    ...
    \begin{subcaptiongroup}
        \begin{tabular}{c@{\hspace{0pt}}c}
            \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_5/Circular_tri3_q1_64_64.png}}
            & \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_5/Circular_tri3_q11_64_64.png}}\\
            mix-Tri3($r=3:2$)&mix-Tri3($r=r_{opt}$)\\
            \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_5/Circular_tri6_q1_32_32.png}}
           & \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_5/Circular_tri6_q11_32_32.png}} \\
           mix-Tri6($r=3:2$)&mix-Tri6($r=r_{opt}$) \\
        \end{tabular}
    \end{subcaptiongroup}
...
\begin{figure}[!h]
    ...
    \begin{subcaptiongroup}
        \begin{tabular}{c@{\hspace{0pt}}c}
            \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_5/Circular_quad4_q1_64_64.png}}
            & \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_5/Circular_quad4_q11_64_64.png}}\\
            mix-Quad4($r=3:2$)&mix-Quad4($r=r_{opt}$)\\
            \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_5/Circular_quad8_q1_32_32.png}}
           & \raisebox{-0.7\height}{\includegraphics[width=0.45\textwidth]{figures/ch_5/Circular_quad8_q11_32_32.png}} \\
           mix-Quad8($r=3:2$)&mix-Quad8($r=r_{opt}$) \\
        \end{tabular}
    \end{subcaptiongroup}
