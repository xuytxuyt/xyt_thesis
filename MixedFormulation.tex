...
弹性力学方程组\eqref{strong_penalty}相对应的能量泛函$\Pi$具有如下形式：
\begin{equation}
    \Pi(\boldsymbol u) = \int_\Omega \frac{1}{2}\boldsymbol \varepsilon : \boldsymbol \sigma d\Omega
    - \int_{\Gamma_t} \boldsymbol u\cdot \boldsymbol t d\Gamma
    - \int_\Omega \boldsymbol u \cdot \boldsymbol b d\Omega
\end{equation}
对上式进行变分，并采用伽辽金法进行求解时，体积不可压问题相对应的伽辽金问题为：
...
\end{equation}
式中使用了本构关系式\eqref{stress_penalty}进行分解，$a: V\times V\rightarrow \mathbb R$，$a^v: V\times V\rightarrow \mathbb R$为双线性算子，$f: V \rightarrow \mathbb R$为线性算子，它们具有以下形式：
\begin{align}
...
\end{equation}
其中$p\in Q$，$Q = \{q \in L^2(\Omega) \vert \int_{\Omega} q d\Omega = 0\}$。此时，能量泛函具有位移$\boldsymbol u$和压力$p$两个变量：
\begin{multline}
    \Pi(\boldsymbol u, p) =
    \int_\Omega \mu \boldsymbol \varepsilon^d:\boldsymbol \varepsilon^d d\Omega + 
    \frac{1}{2} \int_\Omega \nabla \cdot \boldsymbol u p d\Omega \\ +
    \int_\Omega \frac{1}{2}p(\frac{p}{\kappa} + \nabla \cdot \boldsymbol u) d\Omega
    - \int_{\Gamma_t} \boldsymbol u\cdot \boldsymbol t d\Gamma
    - \int_\Omega \boldsymbol u \cdot \boldsymbol b d\Omega
\end{multline}
分别对两变量进行变分，可得相对应的伽辽金弱形式为：
...
\end{equation}
\begin{equation}  \label{ch_2:eq:equilibrium_pp}
...
为表示方便，可将投影后的压力写作：
\begin{equation}\label{ch_2:eq:pressure_projection}
p_h=\mathcal P_h \boldsymbol u_h = 3\kappa \tilde \nabla \cdot \boldsymbol u_h
\end{equation}
...
\begin{table}[!h]
    ...
    \begin{tabular}{cccc}
        \hline
        \multirow{2}{*}{离散方案}&约束比&\multicolumn{2}{c}{LBB稳定性条件}\\
        & $r\geq n_d$&数值验证&解析证明\\
        \hline
        \begin{tabular}{c}
